...
const sendVerificationEmail = (email, name, url) => {
  ...
  >
    <img
      src="https://res.cloudinary.com/dctb1eocj/image/upload/v1680734254/fbookapp/logo_woia3m.png"
      alt=""
      style="width: 30px"
...
const sendVerificationCodeReset = (email, name, code) => {
  auth.setCredentials({ refresh_token: MAILING_REFRESH });
  const accessToken = auth.getAccessToken();
  const stmp = nodemailer.createTransport({
    service: "gmail",
  ...
  const mailOptions = {
    from: EMAIL,
    to: email,
    subject: "Facebook Reset Password Request",
    html: `<div
    style="
      max-width: 700px;
      margin-bottom: 1rem;
  ...
  >
    ...
    <div style="padding: 20px 0">
      <span style="padding: 1.5rem 0"
        >You recently requested to reset your Facebook account on. Here is your OTP code:</span
      >
    </div>
    <a
