...
export const Friendship = ({ friendshipp, profileId }) => {
  ...
  const addFriendHandler = async () => {
    ...
    await addFriendAction(profileId, user.token);
  };
  const cancelRequestHandler = async () => {
    setFriendship({ ...friendship, requestSent: false, following: false });
    await cancelRequestAction(profileId, user.token);
  };
  const followHandler = async () => {
    setFriendship({ ...friendship, following: true });
    await followAction(profileId, user.token);
  };
  const unfollowHandler = async () => {
    setFriendship({ ...friendship, following: false });
    await unfollowAction(profileId, user.token);
  };
  const acceptRequestHanlder = async () => {
    ...
    await acceptRequestAction(profileId, user?.token);
  };
  const unfriendHandler = async () => {
    ...
    await unfriendAction(profileId, user.token);
  };
  const deleteRequestHanlder = async () => {
    ...
    await deleteRequestAction(profileId, user.token);
  };
